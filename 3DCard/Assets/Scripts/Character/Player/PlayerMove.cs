...
public class PlayerMove : MonoBehaviour
{
    ...
    [SerializeField]public float costTurnTime = 100f;
    ...
    void Move()
    {
        if (MapMgr.Instance.allUnwalkableCoor.Contains(toWorldCoor))
        {
            toWorldCoor = MapMgr.Instance.WorldPosToWorldCoor(transform.position);
            return;
        }
        transform.DOMove(MapMgr.Instance.WorldCoorToWorldPos(toWorldCoor), 0.2f).SetEase(Ease.OutCubic);
        PlayerTurnTimeManager.Instance.TickReduceTurnTime(costTurnTime);
    }
    private void MoveInternal(bool isForward)
    {
        ...
        if (isToSceneObj)
        {
            bool isFaceToSceneObj = Vector3.Dot((toSceneObj.transform.position - transform.position).normalized, transform.forward) > 0.8;
            if (isFaceToSceneObj)
            {
                if (isForward)
                {
